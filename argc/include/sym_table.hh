...
public:
  ...
  // For composite types
  auto set_base_type(const std::shared_ptr<Type> &base_type) -> void { base_type_ = base_type; }
  auto get_base_type() const -> std::shared_ptr<Type> { return base_type_; }

  // For arrays
  auto set_array_size (const size_t array_size) -> void { array_size_ = array_size; }
  auto get_array_size () const -> size_t { return array_size_; }

  // For structs and maybe classes in future
  auto set_member_types (const std::shared_ptr<SymbolTable> &member_types) -> void { member_types_ = member_types; }
  ...
  bool can_implicitly_cast_to (const Type& target) const;
...
public:
  explicit Scope (std::string  name = "", int level = 0);
