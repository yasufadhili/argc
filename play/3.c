...
void codegen_call(CodeGen *gen, ASTNode *node);
...
void codegen_expression(CodeGen *gen, ASTNode *node) {
    switch (node->type) {
        ...
        case NODE_IDENTIFIER: {
            ...
            Symbol *symbol = NULL;
            if (gen->locals != NULL) {
                symbol = symbol_table_lookup(gen->locals, name);
            }
            
            if (symbol != NULL) {
                // Local variable
                codegen_add(gen, OP_LOAD, symbol->offset);
            } else {
                // Global variable
                Symbol *global = symbol_table_lookup(gen->globals, name);
                if (global == NULL) {
                    fprintf(stderr, "Undefined variable: %s\n", name);
                    exit(1);
                }
                codegen_add(gen, OP_LOAD, global->offset);
            }
            break;
        }
        ...
        case NODE_BINARY_OP: {
            ...
            switch (node->value.binary_op.operator) {
                ...
                case '=': codegen_add(gen, OP_EQ, 0); break;
                case '!': codegen_add(gen, OP_NE, 0); break;
                case '<': 
                    if (node->value.binary_op.operator == '<' && 
                        node->value.binary_op.right->type == NODE_BINARY_OP &&
                        node->value.binary_op.right->value.binary_op.operator == '=') {
                        codegen_add(gen, OP_LE, 0);
                    } else {
                        codegen_add(gen, OP_LT, 0);
                    }
                    break;
                case '>': 
                    if (node->value.binary_op.operator == '>' && 
                        node->value.binary_op.right->type == NODE_BINARY_OP &&
                        node->value.binary_op.right->value.binary_op.operator == '=') {
                        codegen_add(gen, OP_GE, 0);
                    } else {
                        codegen_add(gen, OP_GT, 0);
                    }
                    break;
                case '&': codegen_add(gen, OP_AND, 0); break;
                case '|': codegen_add(gen, OP_OR, 0); break;
            }
            break;
        }
        ...
        default:
            fprintf(stderr, "Unsupported node type in codegen_expression: %d\n", node->type);
            exit(1);
    }
}
...
void codegen_call(CodeGen *gen, ASTNode *node) {
    ...
    for (int i = node->value.call.argument_count - 1; i >= 0; i--) {
    ...
    for (int i = 0; i < gen->function_count; i++) {
        if (strcmp(gen->function_names[i], node->value.call.name) == 0) {
            codegen_add(gen, OP_CALL, gen->function_addrs[i]);
            return;
        }
    }
...
void codegen_statement(CodeGen *gen, ASTNode *node) {
    switch (node->type) {
        ...
        case NODE_DECLARATION: {
            ...
            // Set the offset for the variable
            Symbol *symbol = symbol_table_lookup(gen->locals == NULL ? gen->globals : gen->locals,
                                              node->value.declaration.name);
    ...
    while (vm->pc < gen->code_size) {
        ...
        switch (instr.opcode) {
            ...
            case OP_LOAD: {
                ...
                if (offset < 0 || offset >= vm->stack_capacity) {
                    fprintf(stderr, "Stack access out of bounds in LOAD: offset=%d\n", offset);
                    exit(1);
                }
                vm_push(vm, vm->stack[offset]);
                break;
            }
            ...
            case OP_STORE: {
                int offset = instr.operand;
                if (vm->stack_size <= 0) {
                    fprintf(stderr, "Stack underflow in STORE\n");
                    exit(1);
                }
                if (offset < 0 || offset >= vm->stack_capacity) {
                    fprintf(stderr, "Stack access out of bounds in STORE: offset=%d\n", offset);
                    exit(1);
                }
            ...
            case OP_RET: {
                ...
                int old_fp = vm->stack[--vm->stack_size];
                int return_addr = vm->stack[--vm->stack_size];
            ...
            case OP_PRINT: {
                if (vm->stack_size <= 0) {
                    fprintf(stderr, "Stack underflow in PRINT\n");
                    exit(1);
                }
                int value = vm_pop(vm);
                printf("%d\n", value);
