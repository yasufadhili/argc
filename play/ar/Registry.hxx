...
class KeywordRegistry {
  ...
  std::vector<TokenType> keywords_{
    TokenType::EXIT,
    TokenType::LET,
    TokenType::SET
  };
...
public:
  ...
  [[nodiscard]] auto is_keyword(const TokenType type) const -> bool {
    return std::find(keywords_.begin(), keywords_.end(), type) != keywords_.end();
  }

  [[nodiscard]] auto get_keywords() const -> std::vector<TokenType> {
    return keywords_;
  }
