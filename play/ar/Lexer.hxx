...
public:
  ...
  LexerError(const std::string &msg, const size_t line, const size_t col, const size_t pos)
    : msg_(msg), line_(line), col_(col), pos_(pos) {
    fprintf(stderr, "%s at line %d:%d", msg.c_str(), static_cast<int>(line_), static_cast<int>(col_));
    exit(EXIT_FAILURE);
  }
...
class Lexer {
  ...
  auto throw_error(const std::string &message) const -> void {
    LexerError(message, start_line_, start_column_, start_pos_);
  }
