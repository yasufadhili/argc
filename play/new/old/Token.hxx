...
class Token {
  ...
  static auto token_type_to_string(TokenType type) -> std::string {
    // Implementation depends on TokenType enum
    // This is just a placeholder
    return std::to_string(static_cast<int>(type));
  }

public:
  Token(const TokenType type, const std::string &lexeme, const SourceLocation &location)
    : type_(type), lexeme_(lexeme), location_(location), value_(std::nullopt) {
  }

  // Constructor for character tokens
  Token(const TokenType type, const std::string &lexeme, const SourceLocation &location, char char_value)
    : type_(type), lexeme_(lexeme), location_(location), value_(char_value) {
  }

  // Constructor for string tokens
  Token(const TokenType type, const std::string &lexeme, const SourceLocation &location,
        const std::string &string_value)
    : type_(type), lexeme_(lexeme), location_(location), value_(string_value) {
  }
